...
User.prototype.daftar_user = async (id_rt) => {
    return await sql.query('SELECT user.* FROM user INNER JOIN keluarga ON keluarga.id_keluarga = user.id_keluarga WHERE keluarga.id_rt = ? AND user.aktif = ? ORDER BY user.level_user,user.nama_lengkap',[id_rt,1]);
}

User.prototype.pencarian_user = async (id_rt,key) => {
    return await sql.query('SELECT user.* FROM user INNER JOIN keluarga ON keluarga.id_keluarga = user.id_keluarga WHERE (keluarga.id_rt = ? AND user.aktif = ?) AND (user.nama_lengkap like ? OR user.nik like ?) ORDER BY IF(user.nama_lengkap LIKE ? OR user.nik LIKE ?,1,2),user.level_user,user.nama_lengkap',[id_rt,1,'%'+key+'%','%'+key+'%',key+'%',key+'%']);
}
